...
and/or sending data from/to remote locations.  Data load itself then
is not directly committed to git, making git-annex repositories
lightweight and easily manageable, while still providing requested
data upon your wish.
...
\item[Special remotes] Besides accessing data from other git remotes
  ...
  and Glacier), or just some host (via ssh or rsync)

\item[Association with web origins] Every file could have a list of
  URLs from which this file could be downloaded

\item[Encryption] Data could be uploaded encrypted to remotes,
...
\item[Semi-compatible with GitHub] Since git-annex repository is just
  a git repository, it could be pushed to GitHub.  The content of data
  files though would need to be stored in some \emph{Special remote}
...
See \url{https://git-annex.branchable.com/walkthrough/} for more
...
\item[\texttt{http://data.pymvpa.org/datasets/haxby2001/.git/}]
  De-facto canonical dataset for testing and demonstrating MVPA
  techniques, originating from a 2001 study by Haxby et. al.
\end{description}
...
\item[\href{http://neuro.debian.net}{NeuroDebian}] Through the joint
  ...
  Data \emph{installation} would become as easy as of software.
\end{description}
